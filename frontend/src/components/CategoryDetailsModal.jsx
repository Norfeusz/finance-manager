...
const TransactionFullDetails = ({ transaction }) => (
	<div className='transaction-full-details'>
		<ul>
			...
			<li>
				<strong>Koszt:</strong> {formatCurrency(transaction.cost)}
			</li>
			<li>
				<strong>Opis:</strong> {transaction.description || '-'}
			</li>
			<li>
				<strong>Data:</strong> {transaction.date ? new Date(transaction.date).toLocaleDateString('pl-PL') : '-'}
			</li>
			<li>
				<strong>Notatka:</strong> {transaction.extraDescription || '-'}
			</li>
		</ul>
	</div>
)
...
function CategoryDetailsModal({ isOpen, onClose, categoryName, transactions, onDataChange }) {
	...
	// Deduplikacja transakcji na podstawie stabilnego klucza (id || rowId || date+amount+desc)
	const uniqueTransactions = (() => {
		try {
			const seen = new Set()
			const dedup = []
			for (const t of Array.isArray(transactions) ? transactions : []) {
				const key = `${t.id ?? t.rowId ?? 'noid'}|${t.date ?? ''}|${Number(t.cost ?? t.amount ?? 0).toFixed(2)}|${
					t.description ?? ''
				}`
				if (!seen.has(key)) {
					seen.add(key)
		...
		try {
			...
			if (response.status === 202 && result?.needsConfirmation && result.action === 'reopen_month' && result.month_id) {
				const confirmReopen = window.confirm(
					result.message || `Miesiąc ${result.month_id} jest zamknięty. Otworzyć aby zaktualizować transakcję?`
				)
				if (confirmReopen) {
	...
	const handleDelete = async transaction => {
		const confirmDelete = window.confirm(
			`Czy na pewno chcesz usunąć transakcję: "${transaction.description || 'Brak opisu'}" o wartości ${formatCurrency(
				transaction.cost
			)}?`
		)
		if (confirmDelete) {
			try {
				...
				if (
					...
					const confirmReopen = window.confirm(
						result.message || `Miesiąc ${result.month_id} jest zamknięty. Otworzyć aby usunąć transakcję?`
					)
					if (confirmReopen) {
						const reopenResp = await fetch(`http://localhost:3002/api/months/${result.month_id}/reopen`, {
							method: 'POST',
				...
				<div className='transaction-list'>
					{uniqueTransactions.length > 0 ? (
						uniqueTransactions.map(t => (
							<div
								key={`tx-${t.id ?? t.rowId ?? t.date + '-' + (t.cost ?? t.amount ?? 0)}`}
								className='transaction-list-item-wrapper'>
								<div className='transaction-list-item'>
									...
									<div className='transaction-actions'>
										<button title='Pokaż szczegóły' onClick={() => toggleDetails(t.id)}>
											ℹ️
										</button>
										<button title='Edytuj' onClick={() => handleEdit(t)}>
											✏️
										</button>
										<button title='Usuń' onClick={() => handleDelete(t)}>
											🗑️
										</button>
									</div>
								</div>
