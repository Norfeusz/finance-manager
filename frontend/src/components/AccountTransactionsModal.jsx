...
function AccountTransactionsModal({
	...
	// Znormalizuj datę do klucza YYYY-MM-DD
	const dateKey = d => {
		if (!d) return ''
		const s = String(d)
		...
		return s // zakładamy YYYY-MM-DD
	}

	// Deduplikacja transferów: jeśli dwie pozycje mają ten sam typ=transfer, tę samą datę, opis (Transfer z:/do:) i kwotę – pokaż tylko jedną
	const dedupedTransactions = (() => {
		...
		for (const t of Array.isArray(transactions) ? transactions : []) {
			...
			let key
			if (t?.type === 'transfer' && (desc.startsWith('Transfer z:') || desc.startsWith('Transfer do:'))) {
				key = `transfer|${dateKey(t.date)}|${desc.toLowerCase()}|${Math.abs(amt).toFixed(2)}|${accountName}`
			} else {
				// dla innych typów – nie deduplikujemy agresywnie
				key = `id:${t?.id ?? t?.rowId ?? Math.random()}`
			}
			if (!seen.has(key)) {
				seen.add(key)
				out.push(t)
	...
	return (
		<Modal
			isOpen={isOpen}
			onClose={onClose}
			title={accountName === 'KWNR' ? 'Konto wydatków nieregularnych' : `Przepływy konta: ${accountName}`}>
			<div className='account-transactions-list'>
				...
				{accountName === 'KWNR' && <KwnrAccountView />}
						...
						<>
							<div className='transactions-header'>
								...
								<div className='transaction-actions'>
									{!showAllTransactions && (
										<button className='show-all-button' onClick={onShowAllTransactions}>
											Pokaż wszystkie przepływy
										</button>
									)}
									{showAllTransactions && <span className='all-transactions-label'>Wszystkie transakcje</span>}
								</div>
							</div>
								...
								<tbody>
									{sortedTransactions.map(transaction => {
										...
										if (type === 'income') {
											...
											// Rozpoznaj kierunek po opisie i polach pomocniczych
											const desc = (transaction.description || '').trim()
											const raw = parseFloat(transaction.cost || transaction.amount || 0)
											const isIncoming = desc.startsWith('Transfer z:') || transaction.toAccount === accountName
											const isOutgoing = desc.startsWith('Transfer do:') || transaction.fromAccount === accountName
											if (isOutgoing) {
												amount = -Math.abs(raw)
												amountClass = 'expense-amount'
											} else if (isIncoming) {
												...
												if (transaction.account === accountName && desc === `Transfer do: ${accountName}`) {
													amount = Math.abs(raw)
													amountClass = 'income-amount'
