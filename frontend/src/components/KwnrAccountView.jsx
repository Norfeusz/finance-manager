...
function KwnrAccountView({ transactions: initialTransactions, currentBalance: initialBalance }) {
	...
	const fetchKwnrData = async () => {
		...
		try {
			...
			let data
			try {
				data = await tryFetch('http://localhost:3002/api/transactions/account/KWNR')
			} catch (e1) {
				console.warn('Primary KWNR endpoint failed, trying fallback:', e1.message)
				// Fallback: endpoint z parametrem zapytania wspierany przez backend
				data = await tryFetch('http://localhost:3002/api/transactions?account=KWNR')
			}
	...
	const ensureMonthAndRetry = async (info, originalAction) => {
		try {
			...
			if (info.action === 'create_month' && info.month_id) {
				...
				const resp = await fetch('http://localhost:3002/api/months/ensure', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ month_id: info.month_id, allowCreate: true }),
				})
				if (!resp.ok) {
					alert('Nie udało się utworzyć miesiąca.')
					return false
				}
			...
			if (info.action === 'reopen_month' && info.month_id) {
				...
				if (!proceed) return false
				const resp = await fetch(`http://localhost:3002/api/months/${info.month_id}/reopen`, { method: 'POST' })
				if (!resp.ok) {
	...
	const handleAddExpense = async e => {
		...
		try {
			...
			if (response.status === 202 && result?.needsConfirmation) {
				const retried = await ensureMonthAndRetry(result, async () => {
	...
	const handleDeleteExpense = async expense => {
		...
		if (confirmDelete) {
			try {
				...
				const response = await fetch('http://localhost:3002/api/expenses/delete', {
					method: 'POST',
					headers: {
				...
				if (
					...
					result.action === 'reopen_month' &&
					result.month_id
				) {
					const confirmReopen = window.confirm(
						result.message || `Miesiąc ${result.month_id} jest zamknięty. Otworzyć aby usunąć?`
					)
					if (confirmReopen) {
				...
				if (response.ok) {
					...
					setExpenses(prevExpenses =>
						prevExpenses.map(exp => (exp.id === expense.id ? { ...exp, isDeleting: true } : exp))
					)
	...
	useEffect(() => {
		...
		transactions.forEach(transaction => {
			...
			const isSettlement =
				transaction.type === 'expense' && transaction.description && transaction.description.startsWith('Rozliczenie:')
			const isTransferToKwnr =
				transaction.description === 'Transfer na KWNR' || transaction.category === 'Transfer na KWNR'

			if (
				(transaction.type === 'expense' || transaction.category === 'Wydatek KWNR') &&
				!isSettlement &&
				!isTransferToKwnr
			) {
				...
				console.log(
					`Wydatek ID ${transaction.id}: extra_description = "${transaction.extra_description}", person = "${transaction.person}"`
				)
				...
				(isTransferToKwnr && transaction.type === 'income')
			) {
		...
		try {
			...
			window.dispatchEvent(
				new CustomEvent('kwnr-sc-changed', { detail: { SC: derived.SC, timestamp: derived.timestamp } })
			)
		} catch {
			/* ignore */
		}
	...
	const saveSettlement = async () => {
		...
		if (isNaN(amountNum) || amountNum <= 0) {
			setSettleError('Podaj prawidłową kwotę')
			return
		}
		if (amountNum > availableFunds) {
			setSettleError('Kwota przekracza dostępne środki')
			return
		}
		if (!settleDate) {
			setSettleError('Wybierz datę')
			return
		}
		try {
			setIsLoading(true)
			...
			if (resp.status === 202 && info?.needsConfirmation) {
				const retried = await ensureMonthAndRetry(info, async () => {
					resp = await doPost()
				})
				if (retried) {
					try {
	...
	const handleEditTransfer = transfer => {
		...
		setEditTransfer({
			...
			date:
				typeof transfer.date === 'string' && transfer.date.includes('.')
					? transfer.date.split('.').reverse().join('-')
					: transfer.date?.split('T')[0] || transfer.date,
	...
	const handleDeleteTransfer = async transfer => {
		...
		try {
			setIsLoading(true)
			const resp = await fetch('http://localhost:3002/api/expenses/delete', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
		...
		<div className='kwnr-account-container'>
				...
				<div className='balance-cards'>
					<div className='balance-card gabi'>
						...
						<button
							...
							disabled={isLoading || Math.abs(gabiBalance) < 0.005}
							onClick={() => startSettlement('Gabi')}>
							Rozlicz
						</button>
					</div>
					<div className='balance-card norf'>
						...
						<button
							className='settle-btn'
							disabled={isLoading || Math.abs(norfBalance) < 0.005}
							onClick={() => startSettlement('Norf')}>
							Rozlicz
						</button>
					</div>
					<div className='balance-card total'>
						...
						<div className='amount'>
							{formatCurrency((gabiBalance || 0) + (norfBalance || 0) + (availableFunds || 0))}
						</div>
					</div>
				</div>
			</div>
			...
			{settleTarget && (
				<div
					...
					style={{ marginTop: '12px', border: '1px solid #ddd', padding: '12px', borderRadius: 6 }}>
					...
					<div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
						...
						<div style={{ display: 'flex', gap: '8px' }}>
							<button type='button' className='save-expense-btn' disabled={isLoading} onClick={saveSettlement}>
								{isLoading ? 'Zapisywanie...' : 'Zapisz rozliczenie'}
							</button>
							<button type='button' className='cancel-btn' onClick={cancelSettlement}>
								Anuluj
							</button>
						</div>
					</div>
					...
					<div style={{ marginTop: 6, fontSize: '0.85em', color: '#555' }}>
						Sugestia to deficyt (ujemne saldo) ograniczony do dostępnych środków. Maks: {formatCurrency(availableFunds)}
					</div>
				</div>
			)}
			...
			<div className='kwnr-section expenses-section'>
				<div className='section-header'>
					...
					<div className='section-controls'>
						...
						<button className='add-button' onClick={() => setShowExpenseForm(!showExpenseForm)} disabled={isLoading}>
				...
				{showExpenseForm && (
					<form className='expense-form' onSubmit={handleAddExpense}>
						...
						<div className='form-actions'>
							...
							<button type='button' className='cancel-btn' onClick={() => setShowExpenseForm(false)}>
								Anuluj
							</button>
						</div>
					</form>
							...
							expenses.map(expense => {
								...
								return (
									<tr key={expense.id} className={expense.isDeleting ? 'deleting' : ''}>
										<td>
											{expense.name || expense.description || (expense.category ? expense.category.name : 'Wydatek')}
										</td>
										<td>
											{(() => {
												...
												console.log(
													`Renderowanie wydatku ID ${expense.id}: person=${expense.person}, extra_description=${expense.extra_description}`
												)
												return personValue
											})()}
										</td>
				...
				{showTransferForm && (
					<form className='expense-form transfer-form' onSubmit={handleAddTransfer}>
						<div className='form-group'>
							...
							<select id='transfer-type' name='type' value={newTransfer.type} onChange={handleTransferChange} required>
						...
						<div className='form-actions'>
							...
							<button type='button' className='cancel-btn' onClick={() => setShowTransferForm(false)}>
								Anuluj
							</button>
						</div>
					</form>
					...
					<tbody>
						{transfers.length > 0 ? (
							transfers.map(transfer => {
								const isSettlement =
									transfer.type === 'expense' && transfer.description && transfer.description.startsWith('Rozliczenie:')
								const personFromDesc = isSettlement
									? transfer.destination || (transfer.description.split(':')[1] || '').trim()
									: null
								const displaySource = isSettlement
									? personFromDesc || 'Nieznane'
									: transfer.source_account_name ||
									  (transfer.description && transfer.description.replace('Wpływ z: ', '')) ||
									  transfer.source ||
									  'Nieznane'
								return (
									<tr key={transfer.id}>
						...
						<div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
							{editTransfer.isSettlement && (
								<div className='form-group'>
									<label>Osoba</label>
									<select
										value={editTransfer.person}
										onChange={e =>
											setEditTransfer({
												...editTransfer,
												person: e.target.value,
												description: `Rozliczenie: ${e.target.value}`,
											})
										}>
										<option value='Gabi'>Gabi</option>
										<option value='Norf'>Norf</option>
							...
							<div className='form-group'>
								...
								<input
									...
									onChange={e => setEditTransfer({ ...editTransfer, date: e.target.value })}
								/>
							</div>
							<div className='form-actions' style={{ justifyContent: 'flex-end' }}>
								<button
									...
									className='save-expense-btn'
									disabled={isLoading}
									onClick={handleSaveEditedTransfer}>
