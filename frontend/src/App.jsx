...
function App() {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        const monthsResp = await fetch('http://localhost:3001/api/months');
        if (monthsResp.ok) {
          const monthsData = await monthsResp.json();
          setMonths(monthsData);
          if (monthsData.length && !monthsData.find(m => m.id === selectedMonthId)) {
            const sorted = monthsData.map(m => m.id).sort().reverse();
            setSelectedMonthId(sorted[0]);
          }
        }
        const response = await fetch(`http://localhost:3001/api/transactions?month_id=${selectedMonthId}`);
  ...
  const toggleMonthLock = async () => {
    ...
    try {
      ...
      if (!selectedMonthObj.is_closed) {
        try {
          const r = await fetch(`http://localhost:3001/api/accounts/bills/${selectedMonthObj.id}`);
          if (r.ok) {
            const js = await r.json();
            const hasDeductions = Array.isArray(js.deductions) && js.deductions.length > 0;
            if (!hasDeductions) {
              const proceed = window.confirm(`Chcesz zamknąć miesiąc ${selectedMonthObj.id}, ale nie ma zapisanych odjęć rachunków. Czy na pewno chcesz zamknąć?`);
              if (!proceed) return;
            }
          }
  } catch { /* brak blokady w razie błędu */ }
      }
      ...
      const resp = await fetch(`http://localhost:3001/api/months/${selectedMonthObj.id}/${endpoint}`, { method: 'POST' });
  ...
  const openInitialIncomesModal = async (monthId, plannedBudget) => {
    try {
      const sugResp = await fetch(`http://localhost:3001/api/months/${monthId}/suggested-initial-incomes`);
  ...
  const handleConfirmInitialIncomes = async () => {
    ...
    if (plannedBudget != null && isFinite(plannedBudget) && sum > plannedBudget) {
      const cont = window.confirm(
        `Suma wpływów (${sum.toFixed(2)} zł) przewyższa planowany budżet (${Number(plannedBudget).toFixed(2)} zł). Czy mimo to dodać takie kwoty?`
      );
      if (!cont) return;
    }
    const payload = [
      { flowType:'income', data:{ toAccount:'Gabi', amount:gVal.toFixed(2), from:'Wpływ początkowy', date:`${monthId}-01` } },
      { flowType:'income', data:{ toAccount:'Norf', amount:nVal.toFixed(2), from:'Wpływ początkowy', date:`${monthId}-01` } }
    ];
    try {
      const incResp = await fetch('http://localhost:3001/api/expenses', {
        method:'POST',
  ...
  const addMonth = async () => {
    if (!months.length) {
      ...
      if (!entered) return;
      if (!/^\d{4}-\d{2}$/.test(entered)) { alert('Nieprawidłowy format.'); return; }
      ...
      budget = budget.trim();
      if (budget !== '' && isNaN(parseFloat(budget))) { alert('Nieprawidłowa kwota budżetu'); return; }
      try {
        const resp = await fetch('http://localhost:3001/api/months', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ year:y, month:m, forceCreate:true, budget: budget === '' ? null : parseFloat(budget) }) });
        if (!resp.ok) { alert('Błąd tworzenia miesiąca'); return; }
        setRefreshKey(k=>k+1);
  ...
  await openInitialIncomesModal(entered, budget === '' ? null : parseFloat(budget));
    ...
    const nextDate = new Date(ly, lm-1, 1); nextDate.setMonth(nextDate.getMonth()+1);
    ...
    if (months.find(m=>m.id === entered)) { alert('Taki miesiąc już istnieje.'); return; }
    // sprawdź poprzedni miesiąc względem entered
    const [ny,nm] = entered.split('-').map(Number);
    const prevDate = new Date(ny, nm-1, 1); prevDate.setMonth(prevDate.getMonth()-1);
    ...
    if (prevMonth && !prevMonth.is_closed) {
      const cont = window.confirm(`Poprzedni miesiąc (${prevId}) nie został zamknięty. Czy mimo to utworzyć ${entered}?`);
      if (!cont) return;
    }
    let budget = window.prompt('Jaki zakładasz budżet na ten miesiąc? (zł)', '4200');
    if (budget === null) return; // anulowano
    ...
    if (budget !== '' && isNaN(parseFloat(budget))) { alert('Nieprawidłowa kwota budżetu'); return; }
    try {
      const resp = await fetch('http://localhost:3001/api/months', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ year: ny, month: nm, forceCreate:true, budget: budget === '' ? null : parseFloat(budget) }) });
      if (!resp.ok) { alert('Błąd tworzenia miesiąca'); return; }
      setRefreshKey(k=>k+1);
      setSelectedMonthId(entered);
  // Otwórz modal z dwiema kwotami (jeśli istnieją sugestie)
  ...
  return (
    <div className="App">
      ...
      <div style={{marginBottom:'1rem'}}>
        <label> Miesiąc: 
          <select value={selectedMonthId} onChange={e => setSelectedMonthId(e.target.value)} style={{marginLeft:'0.5rem'}}>
            {months
              .slice()
              .sort((a,b)=> a.id < b.id ? 1 : -1)
              .map(m => (
              <option key={m.id} value={m.id}>{m.id}{m.is_closed ? ' 🔒' : ''}</option>
            ))}
          </select>
        </label>
          ...
          {!loading && !error && (
            <>
              <StatisticsDashboard 
                ...
                onAddMonth={addMonth}
              />
              <ShoppingStats refreshKey={refreshKey} transactions={transactions} onDataChange={refreshData} selectedMonthId={selectedMonthId} />
            </>
          )}
        </div>
      </div>
      {/* Modal: jednoczesna edycja obu kwot wpływów początkowych */}
      <Modal isOpen={initialIncomesModal.open} onClose={handleCancelInitialIncomes} title={`Wpływy początkowe dla ${initialIncomesModal.monthId || ''}`}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', marginTop: '0.5rem' }}>
            ...
            <button onClick={handleConfirmInitialIncomes} style={{ background: '#2c7', color: '#fff', border: 'none', padding: '0.4rem 0.75rem', borderRadius: 4 }}>Potwierdź</button>
          </div>
        </div>
      </Modal>
    </div>
