...
async function getFinancialData() {
	...
	try {
		...
		const monthsResult = await client.query(`
			SELECT id, year, month, planned_budget
			FROM months 
		...
		// Oblicz średnie wydatki по категориях
...
${JSON.stringify(financialData, null, 2)}`
		...
		const subtitle = reportType === 'monthly' ? `Miesiąc: ${month || new Date().toISOString().slice(0, 7)}` :
		                reportType === 'yearly' ? `Rok: ${new Date().getFullYear()}` :
		                reportType === 'investment' ? `Plan Inwestycyjny` : `Raport Niestandardowy`
