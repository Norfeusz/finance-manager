...
const getShoppingStats = async (req, res) => {
	try {
		...
		try {
			...
			const monthResult = await client.query('SELECT id FROM months WHERE year = $1 AND month = $2', [
				currentYear,
				currentMonth,
			])
			...
			const prevMonthResult = await client.query('SELECT id FROM months WHERE year = $1 AND month = $2', [
				previousYear,
				previousMonth,
			])
...
// Średnia sum wydatków z zamkniętych miesięcy
const getAverageMonthlyExpenses = async (req, res) => {
	try {
		const client = await pool.connect()
		try {
			console.log('Obliczanie średniej sum wydatków z zamkniętych miesięcy (uproszczona logika)')

			// 1. Pobierz sumy wydatków kategorii głównych (is_main=true) z tabeli statistics
			const mainCategoriesQuery = `
                SELECT SUM(s.amount) as total_main_categories
                FROM statistics s
                JOIN categories c ON s.category = c.name
                WHERE s.is_open = false 
                    AND s.category != 'Wpływy' 
                    AND c.is_main = true
            `
			const mainCategoriesResult = await client.query(mainCategoriesQuery)
			const mainCategoriesTotal = parseFloat(mainCategoriesResult.rows[0]?.total_main_categories || 0)

			// 2. Pobierz transfery na konto Rachunki (account_id=3)
			const transfersQuery = `
                SELECT SUM(amount) as total_transfers
                FROM transactions 
                WHERE type='transfer' AND account_id=3
            `
			const transfersResult = await client.query(transfersQuery)
			const transfersTotal = parseFloat(transfersResult.rows[0]?.total_transfers || 0)

			// 3. Stała kwota za transfery na rachunki dla styczeń-lipiec 2025
			const archiveMonthsTransfer = 8400 // 7 miesięcy × 1200 PLN

			// 4. Pobierz liczbę zamkniętych miesięcy
			const closedMonthsQuery = `
                SELECT COUNT(DISTINCT month_id) as closed_months_count
                FROM statistics 
                WHERE is_open = false
            `
			const closedMonthsResult = await client.query(closedMonthsQuery)
			const closedMonthsCount = parseInt(closedMonthsResult.rows[0]?.closed_months_count || 0)

			if (closedMonthsCount === 0) {
				return res.json({
					average: 0,
					monthsCount: 0,
					message: 'Brak zamkniętych miesięcy',
				})
			}

			// 5. Oblicz średnią według uproszczonej formuły
			const totalExpenses = mainCategoriesTotal + archiveMonthsTransfer + transfersTotal
			const average = totalExpenses / closedMonthsCount

			res.json({
				average: +average.toFixed(2),
				monthsCount: closedMonthsCount,
				totalExpenses: +totalExpenses.toFixed(2),
				breakdown: {
					mainCategories: +mainCategoriesTotal.toFixed(2),
					archiveMonthsTransfer: archiveMonthsTransfer,
					transfersToRachunki: +transfersTotal.toFixed(2),
				},
				formula: 'main_categories + 8400 + transfers_to_rachunki',
			})
		} finally {
			client.release()
		}
	} catch (error) {
		console.error('Błąd podczas obliczania średniej wydatków:', error)
		res.status(500).json({
...
module.exports = {
	...
	getCategoryAverages,
	clearAveragesCache,
	getCategoryAveragesLast3Months,
